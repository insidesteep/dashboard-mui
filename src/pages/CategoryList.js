...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_user) => _user.uz.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
...
export default function User() {
  ...
  const [orderBy, setOrderBy] = useState('uz');
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = CATEGORIES.map((n) => n.uz);
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, uz, ru, en } = row;
                    const isItemSelected = selected.indexOf(uz) !== -1;
                    ...
                    return (
                      ...
                      >
                        <TableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onChange={(event) => handleClick(event, uz)} />
                        </TableCell>
                        <TableCell align="left">{uz}</TableCell>
