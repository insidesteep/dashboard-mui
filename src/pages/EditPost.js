...
import { showLoadingpostUpdate, postUpdate } from "../redux/actions/post";
...
export default function EditPost() {
  ...
  useEffect(() => {
    const post = posts.data.option.find((post) => post.post_id == postId);

    if (!post) navigate("/404");

    formik.setValues({
      titleUz: post.title_uz,
    ...
    setInitCategory({
      id: post.category_id,
      category_name_uz: post.category_name_uz,
      category_name_ru: post.category_name_ru,
      category_name_en: post.category_name_en,
    });

    setThumbnail((prev) => ({ ...prev, src: post.previewimg }));

    setContent({
      uz: post.description_uz,
  ...
  useEffect(() => {
    const filtered = categories.data.filter((c) => c.id != initCategory.id);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(showLoadingpostUpdate());
      dispatch(postUpdate(newData, () => navigate(-1)));
    },
  });
