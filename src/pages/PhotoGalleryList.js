...
const TABLE_HEAD = [
  ...
  { id: 'images', label: 'Изображения', alignRight: false },
  { id: '' },
];
...
export default function User() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = PHOTOGALLERIES.map((n) => n.name);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - PHOTOGALLERIES.length) : 0;

  const filteredUsers = applySortFilter(PHOTOGALLERIES, getComparator(order, orderBy), filterName);
            ...
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                <UserListHead
                  ...
                  rowCount={PHOTOGALLERIES.length}
